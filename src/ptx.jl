...
function interpreter_optimization_params(@nospecialize(job::CompilerJob{PTXCompilerTarget}))
    return job.target.always_inline ? (inline_cost_threshold=typemax(Int),) : NamedTuple()
end
